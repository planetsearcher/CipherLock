...
const CONTRACTS = ["EncryptedLedger", "SecureMessenger"];
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(contractFilePath)) {
    console.warn(`Contract deployment file ${contractFilePath} not found. Skipping...`);
    if (!optional) {
      process.exit(1);
    }
    return undefined;
...
for (const contractName of CONTRACTS) {
  ...
  if (deployLocalhost && deploySepolia && deploySepolia.abi) {
    if (
  ...
  if (deployLocalhost) {
    deployments[contractName] = {
      localhost: deployLocalhost,
      sepolia: deploySepolia || { abi: deployLocalhost.abi, address: "0x0000000000000000000000000000000000000000" }
    };
  }
}
...
for (const contractName of CONTRACTS) {
  const deployLocalhost = deployments[contractName].localhost;
  const deploySepolia = deployments[contractName].sepolia;
